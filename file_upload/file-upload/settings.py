...
with open(CONFIG_FILE) as res_file:
    ...
    MEDIA_ROOT = json_data[ACTIVE_PROFILE].get("media_root", "")
    CONTENT_ROOT = json_data[ACTIVE_PROFILE].get("content_root", "")
    CDN_URL = json_data[ACTIVE_PROFILE].get("cdn_url", "")
    JSON_DIR = json_data[ACTIVE_PROFILE].get("json_dir", "")
    CONFIG_JSON_DIR = json_data[ACTIVE_PROFILE].get("config_json_dir", "")
    CONFIG_JSON_NAME = json_data[ACTIVE_PROFILE].get("config_json_name", "")
    UNZIP_CONTENT = json_data[ACTIVE_PROFILE].get("unzip_content", "")
    TELEMETRY = json_data[ACTIVE_PROFILE].get("telemetry", "")
    USB_ON_AUTOMOUNT = json_data[ACTIVE_PROFILE].get("usb_on_automount", "")
    SERVER_ROOT = json_data[ACTIVE_PROFILE].get("server_root", "")
    ACCEPTED_EXTNS = json_data[ACTIVE_PROFILE].get("accepted_extensions", "")
    USB_DIR = json_data[ACTIVE_PROFILE].get("usb_dir", "")
